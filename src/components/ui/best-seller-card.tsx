    ...
    <Card className="flex flex-col h-full overflow-hidden bg-white border border-slate-200">
        ...
        <CardContent className="p-4 flex-grow flex flex-col">
            <div className="flex justify-between items-start mb-3">
                <p className="text-lg font-bold text-slate-800">{formatPrice(product.price)}</p>
                <div className="flex items-center gap-1">
                    <Star className="h-4 w-4 text-amber-400 fill-amber-400" />
                    <span className="text-sm text-slate-600 font-medium">{product.rating.toFixed(1)}</span>
                </div>
            </div>
            ...
            {product.moq > 1 && (
                <div className="pt-3 mt-auto">
                    <Badge variant="outline" className="w-full justify-center text-emerald-600 border-emerald-300 bg-emerald-50">
                        ...
                        Bulk orders available (MOQ: {product.moq})
                    </Badge>
                </div>
            )}
        </CardContent>
